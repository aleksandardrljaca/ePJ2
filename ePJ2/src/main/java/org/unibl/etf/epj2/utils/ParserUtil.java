...
public class ParserUtil {
    ...
    public static List<Rent> parseRentDataCSV() {
        ...
        for(String line:lines){
            String[] data=line.split(",(?=(?:[^\\\"]*\\\"[^\\\"]*\\\")*[^\\\"]*$)");
            if(isAllRentDataPresent(data) && isLocationValid(data[3]) && isLocationValid(data[4]) && isVehicleIdValid(vehicles,data[2])){
                String dateTime=data[0];
                LocalDate date=parseDate(dateTime.split(" ")[0]);
                LocalTime time=parseTime(dateTime.split(" ")[1]);
    ...
    private static boolean isCarDataValid(String[] data) {
        return (!data[0].isEmpty() && !data[1].isEmpty() && !data[2].isEmpty()
                && !data[3].isEmpty() && !data[4].isEmpty() && !data[7].isEmpty());
    }
